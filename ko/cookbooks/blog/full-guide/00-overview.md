...
| 기능 | 설명 |
|------|------|
| 공개 타임라인 | 비로그인 사용자도 발행된 글을 타임라인 피드로 열람 |
| 이메일 인증 | 회원가입, 로그인, 토큰 관리 |
| 게시글 CRUD | 작성, 조회, 수정, 삭제, 공개/비공개 전환 |
| 개인화 대시보드 | 로그인 후 내 글 통계, 드래프트 관리, 내 최근 글 |
...
| 상태 | 홈 화면 | 글 작성 | 글 상세 |
|------|---------|---------|---------|
| **비로그인** | 타임라인 피드 (발행글 시간순) | 불가 | 발행글만 읽기 전용 |
| **로그인** | 개인화 대시보드 (내 글 통계) | 가능 | 작성자는 수정/삭제 가능 |
...
- [데이터베이스 개요](../../../database/01-overview.md) — 동적 테이블 개념
- [스토리지 개요](../../../storage/01-overview.md) — 파일 업로드 개념
