...
파일은 bkend 서버를 거치지 않고 S3에 직접 업로드됩니다. 서버 부하 없이 대용량 파일도 효율적으로 처리할 수 있습니다.
...
flowchart LR
    ...
    subgraph "✅ Presigned URL (bkend 방식)"
        A2[클라이언트] -->|URL 요청만| B2[API 서버]
        B2 -->|서명된 URL 반환| A2
        A2 -->|파일 직접 업로드| C2[S3]
    end
```
...
| **성능** | 클라이언트가 S3에 직접 업로드하므로, API 서버가 파일 데이터를 중계하는 병목이 발생하지 않습니다. 업로드 속도가 서버 대역폭에 제한되지 않습니다. |
