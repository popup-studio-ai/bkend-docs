...
export const cartsApi = {
  list(): Promise<PaginatedResponse<CartDto>> {
    return bkendFetch<PaginatedResponse<CartDto>>("/v1/data/carts?limit=100");
  },

  addItem(productId: string, quantity: number = 1): Promise<CartDto> {
    return bkendFetch<CartDto>("/v1/data/carts", {
      method: "POST",
  ...
  updateQuantity(cartId: string, quantity: number): Promise<CartDto> {
    return bkendFetch<CartDto>(`/v1/data/carts/${cartId}`, {
      method: "PATCH",
      body: { quantity },
    });
  },
  ...
  removeItem(cartId: string): Promise<void> {
    return bkendFetch<void>(`/v1/data/carts/${cartId}`, {
      method: "DELETE",
    });
  },
