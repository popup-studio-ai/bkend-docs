...
```mermaid
flowchart LR
    A[클라이언트] -->|TLS 1.2+| B[bkend API]
    B -->|AES-256-GCM| C[저장 데이터]
    B -->|Argon2id| D[비밀번호]
    B -->|SHA-256| E[API 키]
```
...
| 계층 | 방식 | 대상 |
|------|------|------|
| **인프라 암호화** | MongoDB Atlas Encryption at Rest | 모든 저장 데이터 |
| **애플리케이션 암호화** | AES-256-GCM | 데이터베이스 연결 정보 |

MongoDB Atlas는 저장 데이터를 자동으로 암호화합니다. 추가로 민감한 연결 정보는 애플리케이션 레벨에서 AES-256-GCM으로 이중 암호화됩니다.

### 파일 스토리지

| 항목 | 값 |
|------|-----|
| **방식** | AWS S3 서버 사이드 암호화 |
| **대상** | 업로드된 모든 파일 |
...
| 항목 | 값 |
|------|-----|
| **알고리즘** | Argon2id (OWASP 2025 권장 1순위) |
| **메모리** | 64 MiB |
| **반복 횟수** | 3회 |
| **병렬 처리** | 4 스레드 |
| **해시 길이** | 32바이트 |
...
{% hint style="info" %}
💡 Argon2id는 OWASP 비밀번호 해싱 권장 알고리즘 1순위입니다. GPU 브루트포스 공격에 강한 메모리 기반 해싱을 사용합니다.
{% endhint %}
...
|------|-----------|:------:|
| 네트워크 통신 | TLS 1.2+ | — |
| 데이터베이스 | MongoDB Atlas Encryption at Rest | — |
