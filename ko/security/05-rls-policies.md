...
표현식 기반 권한은 Boolean 모델보다 세밀한 제어를 제공합니다. 각 CRUD 작업에 그룹과 조건을 조합하는 표현식 규칙을 설정할 수 있습니다.
...
1. `expressionPermissions`가 설정되면 `permissions`(Boolean 모드)보다 우선 적용
2. `admin` 그룹은 표현식과 관계없이 항상 모든 권한 보유
3. `list`가 표현식에 정의되지 않으면 `read` 표현식이 폴백으로 사용

{% hint style="info" %}
표현식 기반 권한과 Boolean 권한은 완전히 호환됩니다. 점진적으로 마이그레이션할 수 있으며, 표현식이 없는 테이블은 기존 Boolean 모델을 계속 사용합니다.
{% endhint %}
...
컬럼 수준 권한은 각 사용자 그룹이 어떤 필드를 읽거나 쓸 수 있는지를 제어합니다. 테이블 수준 CRUD보다 세밀한 필드 단위 제어를 제공합니다.
...
| 작업 | 동작 |
|------|------|
| **읽기** | 사용자가 읽을 수 없는 필드는 응답에서 자동으로 제외됩니다 |
| **쓰기** | 사용자가 제한된 필드에 쓰기를 시도하면 `403 PERMISSION_DENIED` 에러가 반환됩니다 |

{% hint style="info" %}
💡 명시적으로 컬럼 권한이 설정되지 않은 필드는 테이블 수준 권한을 상속합니다. 시스템 필드(`id`, `createdBy`, `createdAt`, `updatedAt`)는 항상 읽기 가능합니다.
{% endhint %}

### 예시: 사용자 프로필

관리자만 이메일을 읽을 수 있고, 데이터 소유자만 자기소개를 수정할 수 있습니다:
...
행 필터는 조건에 따라 사용자가 볼 수 있는 행을 자동으로 제한합니다. `self` 권한(`createdBy`로 필터링)과 달리, 행 필터는 임의의 필드 조건을 지원합니다.
...
- 각 행 필터는 `expression`(대상)과 `filter`(적용할 조건)으로 구성됩니다
- 여러 개의 일치하는 필터는 `OR` 로직으로 결합됩니다
- `$userId` 변수는 요청자의 사용자 ID로 치환됩니다

### 예시: 콘텐츠 공개 범위

일반 사용자는 공개된 콘텐츠만, 관리자는 모든 콘텐츠를 볼 수 있습니다:
...
**결과:** 일반 사용자는 공개된 데이터와 본인의 초안을 볼 수 있습니다. 미인증 사용자는 공개 콘텐츠만 볼 수 있습니다. 관리자는 모든 데이터를 볼 수 있습니다 (필터 미적용).

{% hint style="warning" %}
⚠️ 행 필터는 **목록 조회** 작업(`GET /v1/data/:tableName`)에만 적용됩니다. 단건 조회(`GET /v1/data/:tableName/:id`)에는 표준 권한 검사가 사용됩니다.
{% endhint %}
...
| `SCOPE_INSUFFICIENT` | 403 | API 키 scope에 요청한 작업이 포함되지 않는 경우 |
