...
| 환경 | 사용할 키 |
|------|----------|
| 프론트엔드 (브라우저, 모바일) | **Public Key** + 사용자 JWT |
| 서버 사이드 (API Routes, 배치) | **Secret Key** |

{% hint style="danger" %}
🚨 **위험** — Secret Key를 클라이언트 코드에 포함하면 브라우저 DevTools나 앱 디컴파일로 노출됩니다. 프론트엔드에서는 반드시 Public Key를 사용하세요.
{% endhint %}
...
- [Public Key vs Secret Key](03-public-vs-secret.md) — 키 종류별 가이드
